...
\textbf{Problem 3B}
```{r False 3B, eval=FALSE, include=FALSE}

alpha <- iMH.output.burned[ , 1]
beta <- iMH.output.burned[ , 2]

mFunc <- function(x_i) {
  exp(alpha + beta * x_i) / (1 + exp(alpha + beta * x_i))
}

newX <- seq(-5, 5, length.out = 1000)
...
twoDRWMH <- function(lprob, # log-probability density kernel
                     ...
                     theta1=c(2.2, 0.3), # default initial configuration
                     n.iter=1000){
  # allocate output space
...
sim <- twoDRWMH(lprob = seir.lp, Sigma = matrix(c(3e-4, 0, 0, 5e-7), 2, 2))
# print(sim)
colnames(sim) <- c("R","S")
...
#sim.burned <- sim[501:length(sim[ ,1]), ]
